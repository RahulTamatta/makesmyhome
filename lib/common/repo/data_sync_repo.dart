...
import 'package:makesmyhome/helper/db_helper.dart';
...
class DataSyncRepo {
  ...
  Future<Response> _fetchResponseFromClient(String uri,
    ...
    if (method == ApiMethodType.get) {
      // Ensure config endpoints include the required configHeader (zoneId: configuration)
      if (uri == AppConstants.configUri ||
          uri.startsWith(AppConstants.configUri)) {
        return apiClient.getData(uri, headers: AppConstants.configHeader);
      }

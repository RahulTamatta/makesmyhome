...
import 'package:makesmyhome/common/widgets/demo_reset_dialog_widget.dart';
import 'package:makesmyhome/feature/booking/widget/booking_ignored_bottom_sheet.dart';
...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    flutterLocalNotificationsPlugin.initialize(initializationsSettings, onDidReceiveNotificationResponse: (payload) async {
      ...
      try{
        // Do not navigate away if a payment flow is in progress
        if (PaymentGuard.inProgress) {
          return;
        }
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if(!ResponsiveHelper.isWeb()){
        // If in payment, show a heads-up notification only and do not navigate
        if (PaymentGuard.inProgress) {
        ...
        if(Get.find<AuthController>().isLoggedIn()) {
          ...
          message.data['channel_id'] == Get.find<ConversationController>().channelId && Get.currentRoute.contains(RouteHelper.chatScreen)){
            ...
            // Suppress intrusive dialog if payment is in progress
            if (!PaymentGuard.inProgress) {
  ...
  static Future<void> showTextNotification(String title, String? body, String orderID, FlutterLocalNotificationsPlugin fln) async {
    const AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'makesmyhome', 'makesmyhome', playSound: true,
  ...
  static Future<void> showBigTextNotification(String title, String? body, String payload, String image, FlutterLocalNotificationsPlugin fln) async {
    ...
    if(!Get.find<AuthController>().isNotificationActive()){
      AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
        "makesmyhomeWithoutsound","makesmyhome without sound", channelDescription:"description",
        playSound: false,
      ...
      AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
        "makesmyhome", 'makesmyhome with sound', channelDescription:"description",
        playSound: true,

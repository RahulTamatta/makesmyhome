...
void customSnackBar(String? message,
  ...
  if(message != null && message.isNotEmpty) {
    ...
    // If context is not available (e.g., early in app start on web), fall back to a minimal snackbar
    if (ctx == null) {
    ...
    if(showDefaultSnackBar){
      ...
      ScaffoldMessenger.of(ctx)..hideCurrentSnackBar()..showSnackBar(SnackBar(

        content:  Material(
          color: Colors.transparent,
          child: Align(
            alignment: ResponsiveHelper.isDesktop(ctx) ? Alignment.topRight : Alignment.bottomCenter,
            child: Container(
        ...
        padding: EdgeInsets.only(bottom: ResponsiveHelper.isDesktop(ctx) ? Dimensions.paddingSizeLarge : 0),
        margin: ResponsiveHelper.isDesktop(ctx)

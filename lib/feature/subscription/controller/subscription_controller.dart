...
class SubscriptionController extends GetxController {
  ...
  Future<void> fetchSubscriptions({bool forceRefresh = false}) async {
    ...
    try {
      ...
      debugPrint(
          '[FETCH_SUBS][CTRL] Preserving ${currentStatuses.length} current subscription statuses');

      // FIXED: If we have fresh user subscription data and subscriptions list, skip unnecessary fetch
      if (_hasUserSubscriptionData &&
          currentStatuses.isNotEmpty &&
          subscriptions.isNotEmpty &&
          !forceRefresh) {
        debugPrint(
            '[FETCH_SUBS][CTRL] Skipping fetch - already have fresh user subscription data and subscription list');
        isLoading(false);
        return;
      }
      ...
      if (_hasUserSubscriptionData && currentStatuses.isNotEmpty) {
        debugPrint(
            '[FETCH_SUBS][CTRL] Using fresh user subscription data - will preserve all statuses');
      }

      List<Subscription> fetchedSubscriptions =
          await subscriptionService.getSubscriptionsWithUserContext();

      // CR-only filtering: keep subscriptions whose linked service is a CR service
      if (Get.isRegistered<CrModeController>() &&
          Get.find<CrModeController>().isCrMode) {
        try {
          final apiClient = Get.find<ApiClient>();
          final ids = fetchedSubscriptions
              .map((s) => s.serviceId?.toString())
              .where((id) => id?.isNotEmpty ?? false)
              .cast<String>()
              .toSet();
          final Map<String, bool> isCrMap = {};
          await Future.wait(ids.map((sid) async {
            try {
              final res = await apiClient
                  .getData('${AppConstants.crServiceDetailsUri}/$sid');
              isCrMap[sid] = res.statusCode == 200 &&
                  res.body is Map &&
                  res.body['response_code'] == 'default_200';
            } catch (_) {
              isCrMap[sid] = false;
            }
          }));
          fetchedSubscriptions = fetchedSubscriptions.where((s) {
            final id = s.serviceId?.toString();
            return id != null && isCrMap[id] == true;
          }).toList();
          debugPrint(
              '[FETCH_SUBS][CTRL] CR mode ON: filtered to ${fetchedSubscriptions.length} CR subscriptions');
        } catch (e) {
          debugPrint('[FETCH_SUBS][CTRL][CR_FILTER_ERR] $e');
        }
      }

      debugPrint(
          '[FETCH_SUBS][CTRL] Received ${fetchedSubscriptions.length} subscriptions from API');
      ...
      for (final sub in fetchedSubscriptions) {
        ...
        if (id.isNotEmpty && !seenIds.contains(id)) {
          ...
          if (subscriptionId != null &&
              currentStatuses.containsKey(subscriptionId)) {
            ...
            debugPrint(
                '[FETCH_SUBS][CTRL] Added subscription ID: $subscriptionId, Name: ${sub.name}, Preserved Status: $preservedStatus');
          } else {
            uniqueSubscriptions.add(sub);
            debugPrint(
                '[FETCH_SUBS][CTRL] Added subscription ID: ${sub.subscriptionId}, Name: ${sub.name}, New Status: ${sub.subscribed}');
          }
        } else {
          debugPrint(
              '[FETCH_SUBS][CTRL] Skipped duplicate subscription ID: $id');
        }
      }

      subscriptions.assignAll(uniqueSubscriptions);
      debugPrint(
          '[FETCH_SUBS][CTRL] Final count: ${uniqueSubscriptions.length} unique subscriptions');
      debugPrint(
          '[FETCH_SUBS][CTRL] Subscription objects: ${uniqueSubscriptions.map((s) => '${s.name} (ID: ${s.subscriptionId})').join(', ')}');
      ...
      if (uniqueSubscriptions.isEmpty && userSubscriptions.isNotEmpty) {
        debugPrint(
            '[FETCH_SUBS][CTRL] API failed but we have user subscriptions, creating fallback list');
        ...
        for (final userSub in userSubscriptions) {
          if (userSub.subscriptionId != null) {
            ...
            debugPrint(
                '[FETCH_SUBS][CTRL] Created fallback subscription: ${fallbackSub.name}');
          }
        }
        ...
        if (fallbackSubscriptions.isNotEmpty) {
          ...
          debugPrint(
              '[FETCH_SUBS][CTRL] Using ${fallbackSubscriptions.length} fallback subscriptions');
          debugPrint(
              '[FETCH_SUBS][CTRL] Fallback subscription objects: ${fallbackSubscriptions.map((s) => '${s.name} (ID: ${s.subscriptionId})').join(', ')}');
        }
      }
      ...
      debugPrint(
          '[FETCH_SUBS][CTRL] Final state - subscriptions: ${subscriptions.length}, loading: ${isLoading.value}');
    } catch (e) {
      ...
      if (subscriptions.isEmpty && userSubscriptions.isNotEmpty) {
        debugPrint(
            '[FETCH_SUBS][CTRL][ERR] API failed but we have user subscriptions, creating fallback list');
        final fallbackSubscriptions = <Subscription>[];
        ...
        for (final userSub in userSubscriptions) {
          if (userSub.subscriptionId != null) {
            ...
            fallbackSubscriptions.add(fallbackSub);
            debugPrint(
                '[FETCH_SUBS][CTRL][ERR] Created fallback subscription: ${fallbackSub.name}');
          }
        }
        ...
        if (fallbackSubscriptions.isNotEmpty) {
          subscriptions.assignAll(fallbackSubscriptions);
          debugPrint(
              '[FETCH_SUBS][CTRL][ERR] Using ${fallbackSubscriptions.length} fallback subscriptions');

          // Update status map with fallback data
          _updateUserSubscriptionStatusPreserving(
              Map<int, bool>.from(userSubscriptionStatus));
        }
      }
      ...
      if (kIsWeb && e.toString().contains('Failed to fetch')) {
        debugPrint(
            'CORS error detected on web - this is expected during development');
  ...
  void _updateUserSubscriptionStatusPreserving(Map<int, bool> currentStatuses) {
    debugPrint(
        '[UPDATE_STATUS][CTRL] Updating user subscription status map (preserving existing)...');
    ...
    for (final entry in currentStatuses.entries) {
      ...
      debugPrint(
          '[UPDATE_STATUS][CTRL] Preserved subscription ID ${entry.key}: subscribed=${entry.value}');
    }

    // Then, add any new subscriptions that weren't in the existing map
    for (final sub in subscriptions) {
      if (sub.subscriptionId != null &&
          !currentStatuses.containsKey(sub.subscriptionId)) {
        final isSubscribed = sub.subscribed ?? false;
        userSubscriptionStatus[sub.subscriptionId!] = isSubscribed;
        debugPrint(
            '[UPDATE_STATUS][CTRL] Added new subscription ID ${sub.subscriptionId}: subscribed=$isSubscribed');
      }
    }

    debugPrint(
        '[UPDATE_STATUS][CTRL] Status map updated with ${userSubscriptionStatus.length} entries');
  ...
  bool isUserSubscribed(int subscriptionId) {
    ...
    debugPrint(
        '[IS_SUBSCRIBED][CTRL] Checking subscription ID $subscriptionId: $isSubscribed');
    return isSubscribed;
  }
  ...
  void forceUIRefresh() {
    debugPrint(
        '[FORCE_REFRESH][CTRL] Forcing UI refresh - subscriptions: ${subscriptions.length}, loading: ${isLoading.value}');
    isLoading(false);
    subscriptions.refresh();
    userSubscriptionStatus.refresh();
    update();
  ...
  Future<void> _saveSubscriptionStatusToCache() async {
    try {
      ...
      if (userId != null && userId.isNotEmpty) {
        ...
        final statusJson = jsonEncode(userSubscriptionStatus
        ...
        debugPrint(
            '[CACHE][SAVE] Cached ${userSubscriptionStatus.length} subscription statuses');
    ...
    try {
            ...
            if (token.isNotEmpty) {
              ...
              if (parts.length == 3) {
                ...
                try {
                  ...
                  debugPrint(
                      '[CACHE][LOAD] Method 2 - Extracted from token: $userId');
                } catch (e) {
      ...
      if (userId.isNotEmpty) {
        ...
        if (cachedData != null) {
          ...
          debugPrint(
              '[CACHE][LOAD] Restored ${restoredStatus.length} subscription statuses from cache');
          ...
          debugPrint(
              '[CACHE][LOAD] No cached subscription status found for key: $cacheKey');
        }
      } else {
      ...
      debugPrint(
          '[CACHE][LOAD][ERR] Failed to load subscription status from cache: $e');
    }
  }
  ...
  Future<void> _clearSubscriptionStatusCache() async {
    try {
      ...
      if (userId != null && userId.isNotEmpty) {
        ...
        debugPrint(
            '[CACHE][CLEAR] Subscription status cache cleared for user $userId');
      }
    } catch (e) {
      debugPrint(
          '[CACHE][CLEAR][ERR] Failed to clear subscription status cache: $e');
    }
  }
  ...
  Future<void> fetchUserSubscriptions() async {
    try {
      isLoading(true);
      debugPrint(
          '[MY_SUBS][CTRL] Fetching user subscriptions via Bearer token...');
      final data = await subscriptionService.getMySubscriptions();
      debugPrint(
          '[MY_SUBS][CTRL] Received ${data.length} user subscriptions from API');
      ...
      List<Subscription> finalUserSubs = uniqueSubscriptions;
      if (Get.isRegistered<CrModeController>() &&
          Get.find<CrModeController>().isCrMode) {
        try {
          final apiClient = Get.find<ApiClient>();
          final ids = finalUserSubs
              .map((s) => s.serviceId?.toString())
              .where((id) => id?.isNotEmpty ?? false)
              .cast<String>()
              .toSet();
          ...
          finalUserSubs = finalUserSubs.where((s) {
            final id = s.serviceId?.toString();
            return id != null && isCrMap[id] == true;
          }).toList();
          debugPrint(
              '[MY_SUBS][CTRL] CR mode ON: filtered user subscriptions to ${finalUserSubs.length}');
        } catch (e) {
          debugPrint('[MY_SUBS][CTRL][CR_FILTER_ERR] $e');
        }
      }

      userSubscriptions.assignAll(finalUserSubs);
      debugPrint(
          '[MY_SUBS][CTRL] Deduplicated ${data.length} -> ${finalUserSubs.length} subscriptions');
      ...
      if (activeSubIds.isEmpty &&
          userSubscriptionStatus.values.any((status) => status == true)) {
        debugPrint(
            '[MY_SUBS][CTRL] Backend API returned empty but local cache shows active subscriptions');
        debugPrint(
            '[MY_SUBS][CTRL] Preserving local subscription status over backend response');
        ...
        for (final subId in activeSubIds) {
          if (subId != null) {
            ...
            debugPrint(
                '[MY_SUBS][CTRL] Marked subscription $subId as subscribed=true');
          }
        }
        ...
        for (final sub in subscriptions) {
          if (sub.subscriptionId != null &&
              !activeSubIds.contains(sub.subscriptionId)) {
            ...
            if (userSubscriptionStatus[sub.subscriptionId] != true) {
              ...
              debugPrint(
                  '[MY_SUBS][CTRL] Updated subscription ${sub.subscriptionId} to subscribed=false');
            } else {
              debugPrint(
                  '[MY_SUBS][CTRL] Preserved cached status for subscription ${sub.subscriptionId}');
            }
          }
        }
      }
      ...
      for (var i = 0; i < subscriptions.length; i++) {
        ...
        if (sub.subscriptionId != null) {
          ...
          subscriptions[i] =
              sub.copyWith(subscribed: isActive, isActive: isActive ? 1 : 0);
          debugPrint(
              '[MY_SUBS][CTRL] Updated subscription ${sub.subscriptionId} to subscribed=$isActive');
        }
      }
  ...
  Future<void> forceUpdateSubscriptionStatus(
      int subscriptionId, bool isSubscribed) async {
    try {
      debugPrint(
          '[FORCE_UPDATE][CTRL] Force updating subscription $subscriptionId to $isSubscribed');
      ...
      for (int i = 0; i < subscriptions.length; i++) {
        if (subscriptions[i].subscriptionId == subscriptionId) {
          subscriptions[i] =
              subscriptions[i].copyWith(subscribed: isSubscribed);
          debugPrint(
              '[FORCE_UPDATE][CTRL] Updated subscription object $subscriptionId');
          break;
        }
      }
      ...
      debugPrint(
          '[FORCE_UPDATE][CTRL] ✅ Force update completed for subscription $subscriptionId');
    } catch (e) {
  ...
  Future<void> _updateSubscriptionStatusAfterPayment(
      int subscriptionId, String userId) async {
    try {
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL] Updating subscription status after payment');
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL] Subscription ID: $subscriptionId, User ID: $userId');

      // Update local state immediately
      userSubscriptionStatus[subscriptionId] = true;
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL] Updated local status for subscription $subscriptionId to true');

      // Update the subscription object in the list
      for (var i = 0; i < subscriptions.length; i++) {
        if (subscriptions[i].subscriptionId == subscriptionId) {
          subscriptions[i] =
              subscriptions[i].copyWith(subscribed: true, isActive: 1);
          debugPrint(
              '[PAYMENT_SUCCESS][CTRL] Updated subscription object $subscriptionId to subscribed=true');
          break;
        }
      }
      ...
      if (existingUserSub == null) {
        ...
        if (mainSub != null) {
          userSubscriptions
              .add(mainSub.copyWith(subscribed: true, isActive: 1));
          debugPrint(
              '[PAYMENT_SUCCESS][CTRL] Added subscription to userSubscriptions list');
        }
      }
      ...
      _hasUserSubscriptionData = true;
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL] Marked user subscription data as fresh');
      ...
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL] UI updated with new subscription status');
      // GREEN SUCCESS LOG
      debugPrint(_green(
          '[SUCCESS][PAYMENT] Subscription $subscriptionId activated for user $userId'));
      ...
      debugPrint(
          '[PAYMENT_SUCCESS][CTRL][ERR] Failed to update subscription status: $e');
    }
  }
  ...
  Future<bool> _createSubscriptionInBackend({
    ...
    try {
      ...
      debugPrint(
          '[moshi mosh] [CREATE_BACKEND_SUB] user_id: $userId, subscription_id: $subscriptionId, transaction_id: $transactionId');

      // Validate input parameters
      if (subscriptionId <= 0) {
        debugPrint(
            '[CREATE_BACKEND_SUB][ERR] Invalid subscription ID: $subscriptionId');
        return false;
      }
      ...
      if (transactionId.isEmpty) {
        debugPrint(
            '[CREATE_BACKEND_SUB][WARN] Empty transaction ID, generating fallback');
      ...
      if (subscription == null) {
        debugPrint(
            '[CREATE_BACKEND_SUB][WARN] Subscription not found in loaded list for ID: $subscriptionId');
        debugPrint(
            '[CREATE_BACKEND_SUB] This may happen if subscriptions haven\'t loaded yet - using fallback data');
        debugPrint(
            '[CREATE_BACKEND_SUB][INFO] Available subscriptions: ${subscriptions.map((s) => s.subscriptionId).toList()}');
        ...
        debugPrint(
            '[CREATE_BACKEND_SUB] Using fallback data: $subscriptionName, Amount: ₹$amount');
      } else {
        amount = double.tryParse(subscription.price ?? '0') ?? 0.0;
        subscriptionName =
            subscription.name ?? 'Subscription ID $subscriptionId';
        debugPrint(
            '[CREATE_BACKEND_SUB] Found subscription: $subscriptionName, Amount: ₹$amount');
      }

      if (amount <= 0) {
        debugPrint(
            '[CREATE_BACKEND_SUB][ERR] Invalid amount: $amount for subscription: $subscriptionName');
        return false;
      }

      debugPrint(
          '[CREATE_BACKEND_SUB] Validated data - Subscription: $subscriptionName, Amount: $amount');
      debugPrint(
          '[CREATE_BACKEND_SUB] Calling backend API to create subscription');
      ...
      if (success) {
        debugPrint(_green(
            '[CREATE_BACKEND_SUB] ✅ Backend subscription created successfully'));
        debugPrint(
            '[moshi mosh] [CREATE_BACKEND_SUB] backend_created: true for subscription_id: $subscriptionId');
        ...
        try {
          ...
          await Future.delayed(Duration(
              milliseconds: 500)); // Brief delay for backend processing
          await fetchUserSubscriptions();
          debugPrint('[CREATE_BACKEND_SUB] Verification fetch completed');
        } catch (verifyError) {
          debugPrint(
              '[CREATE_BACKEND_SUB][WARN] Verification fetch failed: $verifyError');
        ...
        debugPrint(
            '[CREATE_BACKEND_SUB][ERR] ❌ Backend subscription creation failed');
        debugPrint(
            '[moshi mosh] [CREATE_BACKEND_SUB] backend_created: false for subscription_id: $subscriptionId');
        return false;
      }
    } catch (e, stackTrace) {
      debugPrint(
          '[CREATE_BACKEND_SUB][ERR] ❌ Exception during backend subscription creation: $e');
      debugPrint('[CREATE_BACKEND_SUB][ERR] Stack trace: $stackTrace');
      debugPrint(
          '[moshi mosh] [CREATE_BACKEND_SUB] backend_created: false (exception) for subscription_id: $subscriptionId');
      return false;
    }
  }
  ...
  Future<void> handlePaymentSuccess({
    ...
    try {
      debugPrint(
          '\x1B[32m[PAYMENT_CALLBACK][CTRL] ✅ Payment success callback received after Razorpay payment\x1B[0m');
      debugPrint(
          '[PAYMENT_CALLBACK][CTRL] Subscription ID: $subscriptionId, User ID: $userId, Transaction ID: $transactionId');
      debugPrint(
          '[moshi mosh] [SUBSCRIPTION_MADE] user_id: $userId, subscription_id: $subscriptionId, transaction_id: ${transactionId ?? 'unknown'}');

      // Validate that we have the required data
      if (subscriptionId <= 0) {
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL][ERR] Invalid subscription ID: $subscriptionId');
        return;
      }
      ...
      if (subscription == null) {
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL][WARN] Subscription not found in loaded list for ID: $subscriptionId');
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL] This may happen if subscriptions haven\'t loaded yet - proceeding with payment processing');
        ...
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL] Using fallback data: $subscriptionName, Amount: ₹$amount');
      } else {
        amount = double.tryParse(subscription.price ?? '0') ?? 0.0;
        subscriptionName =
            subscription.name ?? 'Subscription ID $subscriptionId';
        debugPrint(
            '\x1B[32m[PAYMENT_CALLBACK][CTRL] Found subscription: $subscriptionName, Amount: ₹$amount\x1B[0m');
      }

      // CRITICAL FIX: Call the backend subscription creation API first
      debugPrint(
          '\x1B[32m[PAYMENT_CALLBACK][CTRL] Step 1: Creating subscription in backend via API\x1B[0m');
      ...
      if (subscriptionCreated) {
        debugPrint(
            '\x1B[32m[PAYMENT_CALLBACK][CTRL] ✅ Step 1 SUCCESS: Backend subscription created successfully\x1B[0m');
        debugPrint(
            '[moshi mosh] [BACKEND_SUBSCRIPTION_CREATED] success: true, subscription_id: $subscriptionId');
      } else {
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL] ❌ Step 1 FAILED: Backend subscription creation failed, proceeding with local update');
        debugPrint(
            '[moshi mosh] [BACKEND_SUBSCRIPTION_FAILED] success: false, subscription_id: $subscriptionId');
      }

      // CRITICAL FIX: Update subscription status immediately and aggressively
      debugPrint(
          '\x1B[32m[PAYMENT_CALLBACK][CTRL] Step 2: Updating local subscription status\x1B[0m');
      ...
      debugPrint(
          '\x1B[32m[PAYMENT_CALLBACK][CTRL] Step 3: Saving to cache\x1B[0m');
      userSubscriptionStatus[subscriptionId] = true;
      await _saveSubscriptionStatusToCache();
      debugPrint(
          '\x1B[32m[PAYMENT_CALLBACK][CTRL] ✅ Step 3 SUCCESS: Status saved to cache\x1B[0m');
      ...
      for (var i = 0; i < subscriptions.length; i++) {
        if (subscriptions[i].subscriptionId == subscriptionId) {
          subscriptions[i] =
              subscriptions[i].copyWith(subscribed: true, isActive: 1);
          debugPrint(
              '[PAYMENT_CALLBACK][CTRL] Force updated subscription object $subscriptionId to subscribed=true');
      ...
      if (!subscriptionObjectUpdated && subscriptions.isEmpty) {
        debugPrint(
            '[PAYMENT_CALLBACK][CTRL] Subscriptions not loaded yet - status will be updated when they load');
      }
        ...
        if (mainSub != null) {
          final activeSub = mainSub.copyWith(
            ...
            endDate: DateTime.now()
                .add(Duration(days: mainSub.duration ?? 30))
                .toIso8601String(),
          );
          userSubscriptions.add(activeSub);
          debugPrint(
              '[PAYMENT_CALLBACK][CTRL] Added active subscription to userSubscriptions list');
        } else {
          // Create a minimal subscription object if the main subscription isn't loaded
          debugPrint(
              '[PAYMENT_CALLBACK][CTRL] Creating minimal subscription object for userSubscriptions');
      ...
      update();
      debugPrint(
          '[moshi mosh] [SUBSCRIPTION_MADE] ui_updated: true for subscription_id: $subscriptionId');
      ...
      debugPrint(
          '[PAYMENT_CALLBACK][CTRL] Payment success handled successfully');
      debugPrint(
          '[PAYMENT_CALLBACK][CTRL] Final subscription status for $subscriptionId: ${userSubscriptionStatus[subscriptionId]}');
      debugPrint(
          '[moshi mosh] [SUBSCRIPTION_MADE] final_status subscription_id: $subscriptionId => ${userSubscriptionStatus[subscriptionId] == true}');
    } catch (e) {
      debugPrint(
          '[PAYMENT_CALLBACK][CTRL][ERR] Failed to handle payment success: $e');
    }
  }
//     isLoading(true);
//     final success = await subscriptionService.subscribeToService(

...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> loginWithSocialMedia(
    ...
    debugPrint('AuthController: Starting loginWithSocialMedia for ${socialLogInBody.email}');
    _isLoading = true;
    update();
    
    try {
      Response response = await authRepo.loginWithSocialMedia(socialLogInBody);
      debugPrint('AuthController: Social login response status: ${response.statusCode}');
      debugPrint('AuthController: Social login response body: ${response.body}');
      _isLoading = false;
      ...
      debugPrint('AuthController: Social login error: $e');
      _isLoading = false;
      callback(false, '', 'Login failed: $e', null, null, null,
          socialLogInBody.userName, socialLogInBody.email);
      update();
    }
  }
  ...
  Future<void> resetPassword(String identity, String identityType, String otp,
      String password, String confirmPassword, int isFirebaseOtp) async {
    _isLoading = true;
    update();
    Response? response = await authRepo.resetPassword(
        identity, identityType, otp, password, confirmPassword, isFirebaseOtp);
  ...
  bool clearSharedData({Response? response}) {
    // Reset subscription state so UI reflects guest state immediately
    if (Get.isRegistered<SubscriptionController>()) {
      try {
        Get.find<SubscriptionController>().resetSubscriptionState();
      } catch (_) {}
    }
  ...
  // Initialize GoogleSignIn with error handling for missing client ID (Web)
  GoogleSignIn? _googleSignIn;

  void _initializeGoogleSignIn() {
    try {
      if (kIsWeb) {
        // For web, clientId is required. Read from AppConstants.
        final String clientId = AppConstants.googleWebClientId;
        if (clientId.isEmpty) {
          debugPrint('GoogleSignIn (web) not initialized: missing googleWebClientId. Set AppConstants.googleWebClientId and <meta name="google-signin-client_id" /> in web/index.html');
          _googleSignIn = null;
          return;
        }
  ...
  Future<void> socialLogin() async {
    ...
    if (_googleSignIn == null) {
      debugPrint('AuthController: GoogleSignIn instance is null');
      if (kIsWeb) {
        customSnackBar('Google Sign-In not configured for Web. Please set googleWebClientId in AppConstants and meta tag in web/index.html');
      } else {
        customSnackBar('Google Sign-In is unavailable at the moment.');
      }
      return;
    }
    
    try {
      debugPrint('AuthController: Attempting Google Sign-In...');
      googleAccount = (await _googleSignIn!.signIn());
      debugPrint('AuthController: Google Sign-In result: ${googleAccount?.email}');
      
      if (googleAccount != null) {
        auth = await googleAccount?.authentication;
        debugPrint('AuthController: Google authentication token obtained: ${auth?.accessToken != null}');
      } else {
        debugPrint('AuthController: Google Sign-In was cancelled or failed');
      }
    } catch (e) {

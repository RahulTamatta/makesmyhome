import 'dart:convert';
import 'package:makesmyhome/common/widgets/confirmation_widget.dart';
import 'package:makesmyhome/feature/checkout/model/payment_response_model.dart';
...
class CheckOutController extends GetxController implements GetxService{
  ...
  Future<void> placeBookingRequest({
    ...
    // Show booking placement progress
    ...
    if(Get.find<CartController>().cartList.isNotEmpty){
      ...
      if(response.statusCode == 200 && response.body["response_code"] == "booking_place_success_200"){
        ...
        // Refresh cart state after successful booking
        await Get.find<CartController>().getCartListFromServer();

        if(paymentMethod != "offline_payment"){
          // Close booking progress dialog
        ...
        // Close booking progress dialog on error
        if (Get.isDialogOpen == true) {
          Get.back();
        }
      ...
      // Close booking progress dialog if cart is empty
      if (Get.isDialogOpen == true) {
        Get.back();
      }

...
class ServiceController extends GetxController implements GetxService {
  ...
  // Future<void> getSomeServiceList(int offset, bool reload) async {
  //   // Get all services
  //   await getAllServiceList(offset, reload);
  //
  //   // Filter services directly from _allService list
  //   if (_allService != null) {
  //     // Filter services based on their availability status
  //     _filteredServiceList = _allService!
  //         .where((service) => service.isAvailable == 1)
  //         .toList();
  //
  ...
  Future<void> debugClearServiceCache() async {
    try {
      ...
      debugPrint('ServiceController: Error clearing service cache: $e');
    }
  }

  /// Debug method to force refresh all services
  Future<void> debugForceRefreshServices() async {
    debugPrint('ServiceController: Forcing refresh of all services...');
    ...
    await getAllServiceList(1, true);
    await getPopularServiceList(1, true);
    await getTrendingServiceList(1, true);
    await getRecommendedServiceList(1, true);
    await getFeatherCategoryList(true);

    if (Get.find<AuthController>().isLoggedIn()) {
      await getRecentlyViewedServiceList(1, true);
    }

    debugPrint('ServiceController: Debug refresh completed');
  }

  void debugPrintServiceLists() {
    debugPrint("=== SERVICE LISTS DEBUG ===");

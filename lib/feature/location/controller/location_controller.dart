...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(String lat, String long, bool markerLoad,
    ...
    // Debug logging to inspect backend behavior
    try {
      log('getZone() -> status: ${response.statusCode}');
      log('getZone() -> body: ${response.body}');
    } catch (_) {}
    ...
    try {
      if (response.statusCode == 200 &&
        ...
        // Store zone ID in SharedPreferences for HomeScreen detection
        try {
          Get.find<SharedPreferences>().setString(AppConstants.zoneId, _zoneID);
          debugPrint('LocationController: Zone ID stored in SharedPreferences: $_zoneID');
        } catch (e) {
          debugPrint('LocationController: Failed to store zone ID: $e');
        }
      ...
      _inZone = false;
      responseModel = ZoneResponseModel(
          false, 'Unexpected error: $e', '', totalServiceCountInZone);
    }
    if (!isLoading) {
      _isLoading = false;
    }
  ...
  Future<AddressModel> setLocation(String placeID, String address,
    ...
    if (response.statusCode == 200) {
      ...
      latLng = LatLng(placeDetails.content?.location?.latitude ?? 0,
    ...
    if (text.isNotEmpty) {
      try {
        ...
        if (response.statusCode == 200 &&
            ...
            response.body['content'] != null &&
            response.body['content']['suggestions'] != null) {
          _predictionList = [];
          response.body['content']['suggestions']
              .forEach((prediction) =>
          ...
          // Log unexpected or null body to assist debugging
          try {
            log('searchLocation unexpected response: status=${response.statusCode}, body=${response.body}');
          } catch (_) {}
          _predictionList = [];
        }
      } catch (e) {
        try {
          log('searchLocation failed: $e');
        } catch (_) {}
        _predictionList = [];
      }
    }
  ...
  void updateSelectedServiceLocationType(
      {ServiceLocationType? type, bool shouldUpdate = true}) {
    if (type != null) {

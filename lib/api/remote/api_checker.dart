...
class ApiChecker {
  static void checkApi(Response response, {bool showDefaultToaster = true}) {
    ...
    if (kIsWeb && Get.context == null) {
      debugPrint(
          'API Error (${response.statusCode}): ${response.body} - Skipping snackbar on web');
      return;
    }
    ...
    if (response.statusCode != null &&
      ...
      debugPrint(
          'API Redirect (${response.statusCode}): ${response.statusText}');
      return;
    }
    if (response.statusCode == 401) {
      ...
      customSnackBar("${response.statusCode!}".tr,
        ...
        body['errors'] != null) {
      try {
        customSnackBar("${body['errors'][0]['message']}",
            showDefaultSnackBar: showDefaultToaster);
      } catch (_) {
        customSnackBar("${response.statusCode!}".tr,
            showDefaultSnackBar: showDefaultToaster);
